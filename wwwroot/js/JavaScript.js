...
function check() {
    ...
    if (depth <= 0 || depth > 140) {
        error.push("Your first dive must be between 1 and 140 ft.");
    ...
    // Errors
    if (first_group == "") {
        error.push("The time/depth of your first dive is beyond<br />the no-decompression table limits.<br /><br />Please enter a shallower depth and/or shorter dive time.");
        time_label.className = "error";
        time_field.focus();
        output_errors(error);
        return false;
    ...
    printed_answer = "Pressure group after the first dive is " + first_group.toUpperCase() + ".<br />Pressure group after the surface interval is " + second_group.toUpperCase() + ".";
    ...
    if ((depth > 90) || (depth > 80 && time > 21) || (depth > 70 && time > 25) || (depth > 60 && time > 33) || (depth > 50 && time > 47) || (depth > 40 && time > 63) || (depth > 35 && time >= 104) || (time > 139)) {
        printed_answer = printed_answer + "<br /><br />NOTE: <i>Safety stop required after first dive</i>.";
    }

    if ((first_group == "w") || (first_group == "x")) {
        printed_answer = printed_answer + "<br /><br />NOTE: <i>Minimum surface interval is 1 hour (WXYZ rule)</i>.";
    }

    if ((first_group == "y") || (first_group == "z")) {
        printed_answer = printed_answer + "<br /><br />NOTE: <i>Minimum surface interval is 3 hours (WXYZ rule)</i>.";
    }
